...
function inventory(arr1, arr2) {
    ...
    var inventoryObj = {};
    arr1.forEach(function(x){inventoryObj[x[1]] = x[0];});
    arr2.forEach(function(x){
        if(!inventoryObj.hasOwnProperty(x[1]))
            inventoryObj[x[1]] = x[0];
        else
            inventoryObj[x[1]] += x[0];        
    });
    
    arr1 = Object.keys(inventoryObj).sort().map(function(x){
        return [inventoryObj[x],x];
    });
...
//inventory([[21, "Bowling Ball"], [2, "Dirty Sock"], [1, "Hair Pin"], [5, "Microphone"]], [[2, "Hair Pin"], [3, "Half-Eaten Apple"], [67, "Bowling Ball"], [7, "Toothpaste"]]).length ; 
